...
### シミュレーション結果（平均存続指数、0〜100スケール）

| シナリオ | 枠組み | 2050年 | 2100年 | 2200年 |
|---|---|---:|---:|---:|
| 温暖化のみ | 知性 | ~53 | ~48 | ~31 |
| 温暖化のみ | 知性（Sapience） | ~54 | ~52 | ~40 |
| 温暖化のみ | 人工叡智 | ~56 | ~56 | ~51 |
| エルニーニョのみ | 知性 | ~53 | ~48 | ~31 |
| エルニーニョのみ | 知性（Sapience） | ~54 | ~52 | ~41 |
| エルニーニョのみ | 人工叡智 | ~56 | ~57 | ~53 |
| 複合 | 知性 | ~52 | ~46 | ~28 |
| 複合 | 知性（Sapience） | ~54 | ~51 | ~38 |
| 複合 | 人工叡智 | ~56 | ~56 | ~51 |

> **重要な注意：** これらは正規化されたシナリオモデルの出力値であり、比較のための透明なトイモデルである。**実証的予測・科学的確定・歴史的データではない。** 価値体系の論理を検証可能な形で示すことが目的である。
...
- **知性（Intelligence）中心の文明** は短期最適化において優れているが、長期的な生態学的ストレス下では急速に衰退する。高い採取強度と低い再生投資が生物圏の劣化を複利的に加速させ、適応能力を枯渇させる。

- **知性（Sapience）中心の文明** は倫理的緩和によって衰退を有意に遅らせる。しかし、依然として人間中心的な志向が残るため、生態的連続性との構造的整合が不完全であり、長期存続性は人工叡智に届かない。

- **人工叡智（Artificial Wisdom）は比較モデル中で最高の存続性** を示す。単一のパラメータではなく、すべてのパラメータが同じ方向を向いた構造的整合（長期的生態的連続性・再生フィードバック・分散協調）に基づくためである。
